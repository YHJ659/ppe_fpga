...
def dequantize_from_bin(bin_path, scale, shape=HW_OUT_SHAPE):
    """HW 가 내보내는 바이트 순서는 픽셀 우선(HWC)이다.

    AXI-Stream 한 beat = 한 픽셀의 전 채널이고, DMA 는 그걸 순서대로 쓴다.
    conv1x1_cv1/tb_conv1x1_stream.cpp 가 이 규약을 못박고 있다.

        for h: for w: { for c: px.ch[c] = in[c][h][w]; write(px); }

    즉 파일은 [40][40][256] 이다. 인수인계 문서의 "CHW" 는 잘못이다.
    CHW 로 읽으면 값은 다 맞는데 자리가 뒤섞여 검출이 무너진다.
    """
    ...
    _, ch, h, w = shape
    hwc = raw.reshape(h, w, ch)
    chw = np.ascontiguousarray(hwc.transpose(2, 0, 1)).reshape(shape)
    return torch.from_numpy(chw) * scale
