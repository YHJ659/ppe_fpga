...
| split | in_s | y0_s, y1_router_s, y1_concat_s | y1을 라우터용/concat용 2벌로 복제, 출력 3개 |
...
| split | `split_channel_stream_dup.cpp` | 계산 없음, y1을 라우터용/concat용 2벌로 복제 | 1,602 | 119 | 0 | 0 |
...
- [ ] 데이터 경로: DMA(읽기) → conv1x1(cv1) → bn_silu_128 → split(y0, y1_router_s, y1_concat_s) → bottleneck_router ↔ [conv3x3, bn_silu_64, residual_add] → concat(y0, y1_concat_s, y2, y3) → conv1x1(cv2) → bn_silu_128 → DMA(쓰기), 전부 AXI-Stream으로 직결 (4.1절 라우터 포트표 참고)
