...
# model.6.cv2 (C2f 블록의 cv2, 1x1 Conv, concat 직후)의 실제 가중치 추출
weight_tensor = model.model[6].cv2.conv.weight.detach()  # shape: [128, 256, 1, 1]
...
# model.6.cv2에 실제로 들어가는 입력(concat 출력 feature map) 캡처
captured_input = {}
...
input_tensor = captured_input['data'][0]  # [256, 40, 40] (배치 차원 제거)
...
# 파일로 저장 (cv2 전용 폴더에)
import os
os.makedirs("conv1x1_cv2", exist_ok=True)
input_int8.numpy().astype(np.int32).flatten().tofile("conv1x1_cv2/input.bin")
weight_int8.numpy().astype(np.int32).flatten().tofile("conv1x1_cv2/weight.bin")
output_golden.numpy().flatten().tofile("conv1x1_cv2/golden_output.bin")
 
print("저장 완료: 실제 model.6.cv2 가중치 기반 input.bin, weight.bin, golden_output.bin")