...
#pragma HLS UNROLL
        ...
        float mult_tmp  = x_float * bn_scale[c];
        float bn_out;
        // ★ 이번 실험: 부동소수점 덧셈은 정수와 프래그마가 다릅니다.
        // 정수는 op=add를 썼지만(conv3x3), float는 op=fadd로
        // 지정해야 합니다. 여기는 채널당 덧셈이 딱 1번뿐이라
        // conv3x3의 144-way 트리와 달리 효과가 제한적일 수 있습니다.
#pragma HLS BIND_OP variable=bn_out op=fadd impl=fulldsp
        bn_out = mult_tmp + bn_shift[c];
