...
Vivado에 올릴 IP는 이 표의 파일들입니다.

| IP | 확정 파일 | 핵심 설정 | Latency | LUT | BRAM | DSP | 검증 상태 |
|---|---|---|---|---|---|---|---|
| conv1x1 (cv1) | `conv1x1_stream.cpp` | 완전 언롤(128-way), local_weight LUTRAM | 213,004 | 18,304 | 0 | 64 | Co-sim Pass |
| conv1x1 (cv2) | `conv1x1_cv2_stream.cpp` | 완전 언롤(256-way), local_weight LUTRAM | 221,192 | 35,740 | 0 | 128 | Co-sim Pass |
| bn_silu_128 | `bn_silu_128_stream.cpp` | 더블버퍼, exp 유닛 1개, 캐싱 없음(매번 PS가 scale/shift 갱신) | 204,860 | 14,312 | 0 | 25 | Co-sim Pass |
| bn_silu_64 | `bn_silu_64_stream.cpp` | 위와 동일(64채널) | 102,460 | 8,807 | 0 | 25 | Co-sim Pass |
| split | `split_channel_stream.cpp` | 계산 없음, 단순 분배 | 1,602 | 110 | 0 | 0 | Co-sim Pass |
| residual_add | `residual_add_stream.cpp` | 더블버퍼, 채널 직렬 처리 | 102,433 | 5,907 | 0 | 13 | Co-sim Pass |
| concat | `concat_channel_stream.cpp` | DATAFLOW 제거(순차 실행), 3버퍼 ARRAY_RESHAPE(factor=4), layer_id 불필요 | 512,033 | 12,710 | 150 | 8 | Co-sim Pass |
| conv3x3 | `conv3x3_stream_icuf16_v2_weightbram.cpp` | IC_UF=16(acc_partial 재귀 제거), BIND_OP(acc/sum→DSP), local_weight BRAM, win_s/linebuf LUTRAM, layer_id 캐싱 | 1,249,310 | **31,776** | **144** | 335 | C Sim Pass, **Co-sim 미실행** |

**주의**: `conv3x3_stream_icuf16_v2_weightbram.cpp`는 아직 Co-simulation을 안 돌렸습니다.
Vivado 통합 전에 RTL 레벨 검증을 마치는 걸 권장합니다.

**기각된 실험들** (참고용, 채택 안 함):
- conv3x3/conv1x1의 IC_UF 부분 언롤·BIND_OP 조합 중 위 표에 없는 것들 (conv1x1 계열은 전부 역효과)
- concat을 `m_axi`로 DDR에 연결한 버전 (`concat_channel_stream_ddr.cpp`) — LUT 손해가 더 큼
- concat ARRAY_RESHAPE factor=8 (`concat_channel_stream_reshape8.cpp`) — factor=4 대비 개선 없음
- conv1x1 cv2의 local_weight를 BRAM으로 옮긴 버전 — BRAM 256개로 손해가 너무 큼
...
|---|---|---|---|
| BRAM18 | **294** | 288 | **초과 6** |
| LUT | **127,666** | 117,120 | **초과 10,546** |
| DSP | 598 | 1,248 | 여유 650 |

두 자원 다 근소하게 초과된 상태입니다. HLS 추정치가 실제 Vivado 배치·배선(P&R)
결과보다 보수적으로 나오는 경우가 흔하므로, **이 상태로 일단 Block Design
통합 및 실제 구현까지 진행하고, 실측 리포트로 재확인**하기로 했습니다.
실제 구현에서도 초과가 남으면 아래 순서로 추가 조치합니다.

1. concat 버퍼 중 하나만 미세 조정 (reshape 차원/factor 조합을 더 탐색)
2. conv3x3을 IC_UF=8로 한 단계 더 낮추기 (fps 여유 있음, latency 손실 감수)
3. 위로도 부족하면 입력 해상도 축소 등 아키텍처 레벨 변경 재검토
...
- [ ] PS 펌웨어: 5.4절 순서대로 파라미터 로드 → 트리거를 코드로 구현
- [ ] conv3x3의 Co-simulation 마무리 (Vivado 통합 전 RTL 검증 완료)
- [ ] 실제 구현(합성+P&R) 후 Utilization Report로 BRAM/LUT 재확인
